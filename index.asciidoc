...
Some code samples could be found at https://github.com/vaadin/spring-demo7 as well.
...
=== Spring Views
To support navigation between views, the annotations `@SpringView` and `@ViewContainer` can be used.
...
`(Single)ComponentContainer` should be annotated with `@ViewContainer`. In the simplest cases, this can be the UI class
itself.

Also this view is automatically registered as it has the `@SpringView` annotation, so after annotating the UI with
`@ViewContainer` we would be able to open it by opening http://localhost:8080#!view . However, we'll want to make it
...
If you've created Vaadin projects before, there's nothing new here. Usually Vaadin projects are created with
...
Then select the "Simgle Module Application Project". Set your group and
...
`com.vaadin:vaadin-archetype-application:7.7.3` using Eclipse Maven wizard, or with your preferable IDE,
or with just a command line.
